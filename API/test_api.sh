...
CUENTA_TEST="2093599" # Cambia esto por un número de cuenta real de tu BD para probar
...
echo -e "${YELLOW}Iniciando pruebas de la API...${NC}\n"

# ==========================================
# 1. Obtener datos del alumno (GET)
# ==========================================
echo -e "${YELLOW}Prueba 1: Buscar alumno por número de cuenta ($CUENTA_TEST)...${NC}"
HTTP_STATUS=$(curl -s -o /tmp/resp1.txt -w "%{http_code}" -X GET $BASE_URL/alumnos/$CUENTA_TEST)

if [ "$HTTP_STATUS" -eq 200 ]; then
    echo -e "${GREEN}✅ Éxito: El alumno existe y devolvió datos.${NC}"
else
...
# ==========================================
echo -e "${YELLOW}Prueba 2: Confirmar asistencia del alumno...${NC}"
HTTP_STATUS=$(curl -s -o /tmp/resp2.txt -w "%{http_code}" -X POST $BASE_URL/alumnos/$CUENTA_TEST/asistencia \
    -H "Content-Type: application/json" \
    -d '{
        "id_alumno": "'$CUENTA_TEST'",
        "asistira": 1,
        "num_invitados": 2,
...
HTTP_STATUS=$(curl -s -o /tmp/resp3.txt -w "%{http_code}" -X POST $BASE_URL/alumnos/$CUENTA_TEST/asistencia \
    -H "Content-Type: application/json" \
    -d '{
        "id_alumno": "'$CUENTA_TEST'",
        "asistira": 1,
...
HTTP_STATUS=$(curl -s -o /tmp/resp4.txt -w "%{http_code}" -X POST $BASE_URL/alumnos/$CUENTA_TEST/correo \
    -H "Content-Type: application/json" \
    -d '{
        "id_alumno": "'$CUENTA_TEST'",
...
HTTP_STATUS=$(curl -s -o /tmp/resp5.txt -w "%{http_code}" -X GET $BASE_URL/alumnos/$CUENTA_TEST/estado)
